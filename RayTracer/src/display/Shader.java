...
public class Shader {
    ...
    public static Colour computeLocalIllumination(Vector4 point, Vector4 N,
        ...
        ambient = ambient.timesScalar(Ka);
        ...
        // Compute the diffuse and specular components with respect to each light in the
        // scene
        for (Light light : Scene.lights) {
            Vector4 L = (Vector4.subtract(light.position, point)); // Light vector
            L.normalize();
            double dot = Vector4.dotProduct(L, N);
            // Check if this point is not in the shadow of another object with respect to
            // this light
            ...
            if (dot > 0 && s == null) {
                ...
                Colour diffuse = Scene.colors.get(light.colorIndex); // ***
                diffuse = diffuse.timesScalar(Kd);
                diffuse = diffuse.timesColour(IpLambda);
                diffuse = diffuse.timesScalar(dot);
                ...
                if (dot3 > 0) {
                    specular = Scene.colors.get(light.colorIndex);
                    specular = specular.timesScalar(Math.pow(dot3, n));
                    specular = specular.timesScalar(Ks);
                    specular = specular.timesColour(IpLambda);
                    specular = specular.timesColour(Olambda);
                }
